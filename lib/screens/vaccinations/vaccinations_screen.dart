...
class _VaccinationsScreenState extends State<VaccinationsScreen> {
  ...
  List<Vaccination> _filterVaccinations(List<Vaccination> allVaccinations) {
    final vaccinations = allVaccinations.where((vaccination) => _selectedStatus == null || vaccination.computedStatus == _selectedStatus).toList();
    ...
    return StreamBuilder<List<Vaccination>>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return const LoadingWidget(message: 'Loading vaccinations...');
        }
        ...
        final overdueCount = countOverdueVaccinations(allVaccinations);
        final dueSoonCount = countDueSoonVaccinations(allVaccinations);
        final filteredVaccinations = _filterVaccinations(allVaccinations);
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (filteredVaccinations.isEmpty)
                    ...
                    _MobileVaccinations(vaccinations: filteredVaccinations)
                  else
                    _DesktopVaccinations(vaccinations: filteredVaccinations),
                ],
              ),
            ),
          ),
...
class _DesktopVaccinationRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Padding(padding: const EdgeInsets.symmetric(vertical: 12), child: Row(children: [Expanded(flex: 2, child: _PatientIdentity(vaccination: vaccination)), Expanded(flex: 2, child: Text(vaccination.vaccine, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: AppColors.textPrimary, fontSize: 12))), Expanded(child: _DateText(date: vaccination.administrationDate)), Expanded(child: _DateText(date: vaccination.nextDueDate, emphasized: vaccination.computedStatus != VaccinationStatus.completed)), Expanded(flex: 2, child: BranchBadge(branchName: vaccination.branchId)), Expanded(flex: 2, child: Text(vaccination.veterinarianName, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: AppColors.textSecondary, fontSize: 11))), Expanded(child: _VaccinationStatus(status: vaccination.computedStatus))]));
}
...
class _MobileVaccinationCard extends StatelessWidget {
  ...
  final Vaccination vaccination;
  @override
  Widget build(BuildContext context) => LightGlassPanel(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [PetAvatar(species: _speciesFor(vaccination.petName)), const SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(vaccination.petName, style: const TextStyle(color: AppColors.textPrimary, fontSize: 13, fontWeight: FontWeight.w800)), const SizedBox(height: 3), Text(vaccination.petId, style: const TextStyle(color: AppColors.textSecondary, fontSize: 10))])), _VaccinationStatus(status: vaccination.computedStatus)]), const Divider(height: 24), _MobileDetail(label: 'Vaccine', value: vaccination.vaccine), const SizedBox(height: 12), Row(children: [Expanded(child: _MobileDetail(label: 'Administered', value: _formatDate(vaccination.administrationDate))), Expanded(child: _MobileDetail(label: 'Next due', value: _formatDate(vaccination.nextDueDate)))]), const SizedBox(height: 12), _MobileDetail(label: 'Veterinarian', value: vaccination.veterinarianName), const SizedBox(height: 12), BranchBadge(branchName: vaccination.branchId)]));
}
